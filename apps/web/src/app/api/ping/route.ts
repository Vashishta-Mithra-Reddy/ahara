...
function formatLocalWindowLabel(startMin: number, endMin: number) {
	...
	const sMer = sLocal.endsWith("00")
		? to12h(sLocal).replace(":00 ", " ")
		: to12h(sLocal);
	const eMer = eLocal.endsWith("00")
		? to12h(eLocal).replace(":00 ", " ")
		: to12h(eLocal);
...
async function sendPushNotification(
	...
	try {
		...
		console.log(
			`[FCM] Found ${tokenArray.length} push token(s) for user ${userId}`,
		);
		...
		for (const tokenRecord of tokenArray) {
			...
			if (token) {
				...
				if (success) {
					successCount++;
					console.log(
						`[FCM] ✅ Successfully sent to token: ${token.substring(0, 20)}...`,
					);
				} else {
					console.warn(
				...
				console.warn(
					`[FCM] Invalid token record for user ${userId}:`,
					tokenRecord,
				);
			}
		}
		...
		console.log(
			`[FCM] Sent push notification to ${successCount}/${tokenArray.length} devices for user ${userId}`,
		);
		return successCount > 0;
	} catch (error) {
		console.error(
			`[FCM] Error sending push notification to user ${userId}:`,
			error,
		);
		return false;
	}
...
export async function GET(req: Request) {
	...
	if (!token) {
		...
		return NextResponse.json(
			{ error: "Server configuration error: Missing bearer token" },
			{ status: 500 },
		);
	}
	...
	if (!timestamp || Math.abs(nowSec - parseInt(timestamp)) > 60) {
		console.warn(
			"[PING] Stale or missing timestamp. Now:",
			nowSec,
			"Provided:",
			timestamp,
		);
	...
	if (signature !== expected) {
		console.warn(
			"[PING] Invalid signature. Expected:",
			expected,
			"Received:",
			signature,
		);
	...
	try {
		...
		for (const s of settings) {
			...
			console.log(
				`[PING] User ${userId} local time: ${nowLocal.hour}:${String(nowLocal.minute).padStart(2, "0")} (${nowMinLocal} minutes)`,
			);

			// Local windows from settings
			const logStartMin = hhmmToMinutes(
				normalizeTime((s as any).logReminderTime ?? "20:00"),
			);
			const logDuration = LOG_WINDOW_MIN;
			const logEndMin = (logStartMin + logDuration) % (24 * 60);

			const symptomStartMin = hhmmToMinutes(
				normalizeTime((s as any).symptomCheckTime ?? "22:00"),
			);
			const symptomDuration = SYMPTOM_WINDOW_MIN;
			const symptomEndMin = (symptomStartMin + symptomDuration) % (24 * 60);

			console.log(
				`[PING] User ${userId} log window: ${minutesToHhmm(logStartMin)} - ${minutesToHhmm(logEndMin)}`,
			);
			console.log(
				`[PING] User ${userId} symptom window: ${minutesToHhmm(symptomStartMin)} - ${minutesToHhmm(symptomEndMin)}`,
			);

			// Compute user's local day bounds in UTC using timezone-aware offset
			const localMidnightBaseUtc = new Date(
				Date.UTC(nowLocal.year, nowLocal.month - 1, nowLocal.day, 0, 0, 0),
			);
			const localMidnightOffset = getLocalParts(
				localMidnightBaseUtc,
				tz,
			).offsetMin;
			const startUtc = new Date(
				localMidnightBaseUtc.getTime() - localMidnightOffset * 60000,
			);
			...
			if ((s as any).dailyLogReminder) {
				...
				if (inWindow && !alreadyLogged && !alreadySent) {
					...
					if (success) {
						...
						sentLog++;
						console.log(
							`[PING] ✅ Log reminder sent successfully to user ${userId}`,
						);
					} else {
						console.log(
							`[PING] ❌ Failed to send log reminder to user ${userId}`,
						);
					}
				} else {
					console.log(
						`[PING] Skipping log reminder for user ${userId} (conditions not met)`,
					);
				}
			} else {
			...
			if ((s as any).symptomCheckReminder) {
				...
				if (inWindow && !alreadyLogged && !alreadySent) {
					...
					const windowLabel = formatLocalWindowLabel(
						symptomStartMin,
						symptomEndMin,
					);
					const pushPayload: PushPayload = {
					...
					if (success) {
						...
						sentSymptom++;
						console.log(
							`[PING] ✅ Symptom reminder sent successfully to user ${userId}`,
						);
					} else {
						console.log(
							`[PING] ❌ Failed to send symptom reminder to user ${userId}`,
						);
					}
				} else {
					console.log(
						`[PING] Skipping symptom reminder for user ${userId} (conditions not met)`,
					);
				}
			} else {
		...
		console.log(
			`[PING] Processing complete. Sent ${sentLog} log reminders and ${sentSymptom} symptom reminders`,
		);
