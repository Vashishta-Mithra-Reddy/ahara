...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches
      ...
      .then((keys) =>
        Promise.all(
          keys.map((key) => (key !== CACHE_VERSION ? caches.delete(key) : undefined))
        )
      )
  );
  self.clients.claim();
...
self.addEventListener("fetch", (event) => {
    ...
    event.respondWith(
      caches.match(request).then((cached) => {
        const fetchPromise = fetch(request)
          ...
          .catch(() => cached || fetch(request).catch(() => new Response(null, { status: 504 })));
  ...
  event.respondWith(
    fetch(request).catch(() => caches.match(request).then((cached) => cached || new Response(null, { status: 504 })))
  );
});
